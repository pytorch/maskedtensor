...
from torch.testing import \
    (floating_types, floating_types_and, floating_and_complex_types_and,
     integral_types_and, all_types, all_types_and, all_types_and_complex_and)
...
from torch.testing._internal.common_methods_invocations import \
    (DecorateInfo, SampleInput, OpInfo, UnaryUfuncInfo, BinaryUfuncInfo, ReductionOpInfo,
     ...
     sample_inputs_unary, sample_inputs_i0_i1, reference_lgamma, sample_inputs_logit,
     error_inputs_neg, np_sinc_with_fp16_as_fp32, reference_sigmoid, reference_sign, reference_sgn,
     sample_inputs_add_sub, sample_inputs_elementwise_binary, sample_inputs_comparison_ops)
...
def sample_inputs_clamp_scalar(op_info, device, dtype, requires_grad, **kwargs):
    ...
    for shape, vals in itertools.product(shapes, min_max_vals):
        ...
        output.append(
            SampleInput(tensor.clone().requires_grad_(requires_grad), 
                        args=vals, kwargs={'mask': mask}))
...
additional_op_db.extend([
    ...
    BinaryUfuncInfo('lt',
                    ...
                    always_returns_bool=True,
                    supports_autograd=False,),
    BinaryUfuncInfo('maximum',
                    dtypes=all_types_and(torch.float16, torch.bfloat16, torch.bool),
                    supports_forward_ad=True,
                    supports_fwgrad_bwgrad=True,
                    ref=np.maximum,
                    supports_rhs_python_scalar=False),
    BinaryUfuncInfo('minimum',
                    dtypes=all_types_and(torch.float16, torch.bfloat16, torch.bool),
                    supports_forward_ad=True,
                    supports_fwgrad_bwgrad=True,
                    ref=np.minimum,
