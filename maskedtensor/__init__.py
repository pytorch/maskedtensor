...
UNARY_FNS = [torch.ops.aten.cos,  torch.ops.aten.sqrt, torch.ops.aten.log, torch.ops.aten.exp, torch.ops.aten.pow, torch.ops.aten.sin, torch.ops.aten.clamp]
BINARY_FNS = [torch.ops.aten.add, torch.ops.aten.sub, torch.ops.aten.div, torch.ops.aten.mul, torch.ops.aten.add_, torch.ops.aten.le]
REDUCE_FNS = [torch.ops.aten.sum, torch.ops.aten.mean, torch.ops.aten.amin, torch.ops.aten.amax, torch.ops.aten.prod]
...
def masks_match(a, b):
  if is_masked_tensor(a) and is_masked_tensor(b):
      ...
      mask_b = get_mask(b)
      if VERBOSE:
          print(" mask_a.size(): ", mask_a.size(),
                " mask_b.size(): ", mask_b.size(),
                " mask_a.stride(): ", mask_a.stride(),
                " mask_b.stride(): ", mask_b.stride())
      return (mask_a.dim() == mask_b.dim()) and torch.equal(mask_a, mask_b)
  return True

def masked_tensor_str(data, mask, formatter):
  if data.dim() == 1:
    formatted_elements = [formatter.format(d.item()) if isinstance(d.item(), float) else str(d.item()) for d in data]
    max_len = max(map(lambda x: 8 if x[1] else len(x[0]), zip(formatted_elements, ~mask)))
    return ("[" +
            ", ".join(["--".rjust(max_len) if m else e
                      for (e, m) in zip(formatted_elements, ~mask)]) +
            "]")
  sub_strings = [masked_tensor_str(d, m, formatter)
...
class MaskedTensor(torch.Tensor):
  ...
  def __repr__(self):
    ...
    if self.dim() == 0:
      ...
      if not get_mask(self).item():
        data_formatted = "--"
      return "masked_tensor(" + data_formatted + ", " + str(get_mask(self).item()) + ")"
  ...
  def reduce(cls, fn, data, mask, dim, new_mask, keepdim):
    ...
    if fn is torch.ops.aten.amin:
      max_value = data.max()
      return MaskedTensor(fn(data.masked_fill(~mask, max_value), dim, keepdim),
                          new_mask)
    if fn is torch.ops.aten.amax:
      min_value = data.min()
      return MaskedTensor(fn(data.masked_fill(~mask, min_value), dim, keepdim),
                          new_mask)
    return NotImplemented
  ...
  def matmul(cls, input0, input1, func):
    ...
    if is_masked_tensor(input0):
      return cls.matmul(input0, MaskedTensor(input1, torch.ones_like(input1).bool()), func)
    if is_masked_tensor(input1):
      return cls.matmul(MaskedTensor(input0, torch.ones_like(input0).bool()), input1, func)
    return NotImplemented
  ...
  def __torch_dispatch__(cls, func, types, args, kwargs):
    ...
    if func in UNARY_FNS:
      ...
      if len(kwargs) == 0 and len(args) == 3:
        return MaskedTensor(func(get_data(args[0]), args[1], args[2]), get_mask(args[0]))
    ...
    if func in REDUCE_FNS:
      ...
      if len(args) == 2 or len(args) == 3:
        ...
        return cls.reduce(func, get_data(args[0]), get_mask(args[0]), dim_args, new_mask, keepdim)
      assert len(args) == 1
    ...
    if func is torch.ops.aten._softmax:
      assert len(args) == 3
      assert len(kwargs) == 0
      input_data = get_data(args[0]).masked_fill(~get_mask(args[0]), float("-inf"))
      if VERBOSE:
    ...
    if func in [torch.ops.aten.select, torch.ops.aten.transpose, torch.ops.aten.split]:
      assert len(args) == 3
      assert len(kwargs) == 0
      ...
      if isinstance(result_data, list):
        return tuple(MaskedTensor(di, mi) for (di, mi) in zip(result_data, result_mask))
      return MaskedTensor(result_data, result_mask)
    if func is torch.ops.aten.t:
      ...
      return MaskedTensor(result_data, result_mask)
    if func in [torch.ops.aten.index, torch.ops.aten.expand, torch.ops.aten.view, torch.ops.aten._unsafe_view]:
      assert len(args) == 2
      assert len(kwargs) == 0
      ...
      if func is torch.ops.aten.view:
        return MaskedTensor(torch.ops.aten.view(data, args[1]), torch.ops.aten.view(mask, args[1]))
    ...
    if func in [torch.ops.aten.ones_like]:
      ...
      if VERBOSE:
        ...
        print("res_data.dtype: ", res_data.dtype, " res_data.device: ", res_data.device)
    ...
    if func is torch.ops.aten._softmax_backward_data:
      ...
      if is_masked_tensor(grad_output) and is_masked_tensor(self):
        ...
        grad_data = torch.ops.aten._softmax_backward_data(get_data(grad_output), get_data(output), dim, get_data(self))
    ...
    if func is torch.ops.aten._reshape_alias:
      ...
      return MaskedTensor(get_data(args[0]).reshape(args[1]), get_mask(args[0]).reshape(args[1]))
